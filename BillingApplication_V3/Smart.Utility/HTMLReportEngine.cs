...
namespace Smart.Utility
{
	...
	public class Report : System.Web.UI.Page
	{
        ...
        public string GenerateReport(Boolean IncludeHeader)
        {
            ...
            if (IncludeHeader)
                ...
                htmlContent.Append("<h2>" + ReportTitle + "</h2>");

            WriteSections(IncludeHeader);
            WriteFooter();
		...
		public string SaveReport(string fileName, Boolean includeHeader) 
		{
			try 
			{
				string HTMLContent = GenerateReport(includeHeader);
        ...
        private void WriteTitle()
        {
            ...
            htmlContent.Append("<HTML><HEAD><TITLE>Report - " + reportTitle + "</TITLE>" + newline);
            ...
            htmlContent.Append(" .DetailData  {font-family: Arial; font-size:9pt;  vertical-align:top; width:7in;height:0.35in;} " + newline);
            htmlContent.Append(" .ColumnHeaderStyle  {font-family: " + ReportFont + "; font-size:9pt; vertical-align:top;width:1in; text-align:right;} " + newline);
            htmlContent.Append(" .break{page-break-before: always; } " + newline);
            htmlContent.Append(" table{width: 8in; margin:5px;}  " + newline);
            htmlContent.Append(" td{height:.35in;} " + newline);
            htmlContent.Append("</STYLE>" + newline);
		...
		/// </summary>
		private void WriteSections(Boolean includeHeader) 
		{
			if(sections.Count == 0) 
			{
        ...
        private void WriteSectionHeader(Section section, string sectionValue)
        {
            ...
            if (isFirstHeading)
            {
                htmlContent.Append("<h1 style=\"background-color:Gray;\">");
                isFirstHeading = false;
            }
            else
                htmlContent.Append("<h1 style=\"background-color:Gray; page-break-before: always;\">");
		...
		private Hashtable WriteSectionDetail(Section section, string criteria) 
		{
			...
			try
			{
				...
				foreach(Field field in this.reportFields) 
				{
                    ...
                    htmlContent.Append("  <TD " + cellParams + " Style=\"font-family: Arial; font-size:9pt; vertical-align:top;width:1in; text-align:right;\"><b>" + field.HeaderName + "</b></TD>" + newline);
				}
				htmlContent.Append("</TR>" + newline);
				...
				foreach(DataRow dr in reportSource.Tables[0].Select(criteria))
				{
					...
					foreach(Field field in this.reportFields) 
					{
						...
						cellParams += " ALIGN='" + field.alignment + "' ";
                        htmlContent.Append("  <TD " + cellParams + " VALIGN='top' Style=\"font-family: Arial; color:Black; font-size:9pt;  padding-left:2px; vertical-align:top; width:6.5in;height:0.35in;\">" + dr[field.FieldName] + "</TD>" + newline);
					}
					htmlContent.Append("</TR>" + newline);
					try 
					{
        ...
        /// <param name="section">the section details</param>
        /// <param name="criteria">the section selection criteria</param>
        private Hashtable WriteSectionDetailVertical(Section section, string criteria,string sectionValue)
        {
            Hashtable totalArray = new Hashtable();
            totalArray = PrepareData(totalArray);
            if (section == null)
            ...
            try
            {
                ...
                foreach (DataRow dr in reportSource.Tables[0].Select(criteria))
                {
                    WriteSectionHeader(section, sectionValue);
                    ...
                    foreach (Field field in this.reportFields)
                    {
                        ...
                        {
                            Id = dr[field.FieldName].ToString();
                            //do nothing
                        }
                        else if (field.FieldName == "LineNumber")
                        {
                            ln = dr[field.FieldName].ToString();
                            htmlContent.Append("<tr>" + newline);

                            htmlContent.Append("  <td Style= \"font-family: Arial; color:Black; font-size:9pt; vertical-align:top; width:1.5in;padding-right:2px; text-align:right;font-weight:bold;\"><b>" + field.HeaderName + " : </b></td>" + newline);
                            htmlContent.Append("  <td Style=\"font-family: Arial; color:Black; font-size:9pt;  padding-left:2px; vertical-align:top; width:6.5in;height:0.35in;\">" + lineNo.ToString() + "</td>" + newline);
                            htmlContent.Append("</tr>" + newline);
                        }

                        #region handle image
                        else if (field.FieldName == "ImageName")
                        {
                            ImageList_TBLL objBll = new ImageList_TBLL();
                            DataTable dtImage = objBll.GetAllImageList_T(" where RefId='" + Id + "'");
                            ...
                            if (dtImage.Rows.Count > 0)
                            {
                                foreach (DataRow row in dtImage.Rows)
                                {
                                    ...
                                    if (path != "")
                                    {
                                        ...
                                        htmlContent.Append("</table>" + newline);
                                        htmlContent.Append("  <div Style=\"width: 7in; text-align:center;vertical-align: top;\">");
                                        htmlContent.Append("    <img id=\"img" + imgeId.ToString() + "\" Style=\"max-width: 100%;max-height: 8in; height: auto;  width: auto; text-align:center;vertical-align: top;\" src=\"" + path + "\" ImageAlign=\"Middle\">" + newline);
                                        htmlContent.Append("  </div>" + newline);
                                        htmlContent.Append("  <div Style=\"font-family: Arial; color:Black; font-size:9pt; vertical-align:top; padding-right:2px; font-weight:bold;text-align:center\"><b>" + filename + " </b></div>" + newline);
                                        htmlContent.Append("<table>" + newline);
                                        imgeId++;
                                    }
                                }
                        ...
                        else
                        {
                            ...
                            if (field.FieldName.Contains("Date"))
                            {
                                try
                                {
                                    string dtString = dr[field.FieldName].ToString();

                                    DateTime dt = DateTime.Parse(dtString);
                                    dtString = dt.ToString(culture.DateTimeFormat);

                                    htmlContent.Append("  <td  Style=\"font-family: Arial; color:Black; font-size:9pt;  padding-left:2px; vertical-align:top; width:6.5in;height:0.35in;\">" + dtString + "</td>" + newline);
                                }
                                catch
                                {
                                    htmlContent.Append("  <td  Style=\"font-family: Arial; color:Black; font-size:9pt;  padding-left:2px; vertical-align:top; width:6.5in;height:0.35in;\">" + dr[field.FieldName] + "</td>" + newline);
                                }
                            }
                            else if (field.FieldName.Contains("Price"))
                            {
                                string price = "", currency = "";
                                string strValue = dr[field.FieldName].ToString();
                                try
                                {
                                    ...
                                    price = double.Parse(price, CultureInfo.InvariantCulture).ToString(culture);
                                    strValue = price + " " + currency;

                                    htmlContent.Append("  <td  Style=\"font-family: Arial; color:Black; font-size:9pt;  padding-left:2px; vertical-align:top; width:6.5in;height:0.35in;\">" + strValue + "</td>" + newline);
                                }
                                catch
                                {
                                    strValue = "0";
                                ...
                                htmlContent.Append("  <td  Style=\"font-family: Arial; color:Black; font-size:9pt;  padding-left:2px; vertical-align:top; width:6.5in;height:0.35in;\">" + dr[field.FieldName] + "</td>" +
                                                   newline);
                    ...
                    lineNo++;
                    htmlContent.Append("<tr>" + newline);
                    htmlContent.Append("<td>&nbsp" + newline);
		...
		private Hashtable RecurseSections(Section section, string criteria) 
		{
			...
			foreach(object obj in result) 
			{
                ...
                {
                    sectionTotal = WriteSectionDetailVertical(section, tcriteria, obj.ToString());
		...
		private void GenerateBarChart(string criteria, Section section) 
		{
			...
			try 
			{
				...
				string barTitle = "<TR><TD Style=\"font-family: Arial; color:Black; font-size:9pt; vertical-align:top; width:1.5in;padding-right:2px; text-align:right;font-weight:bold;\" colspan=3 align='CENTER' width=550><B>ChartTitle</B></TD></TR>";
				htmlContent.Append(barTitle.Replace("ChartTitle",section.ChartTitle) + newline);

                string barTemplate = "<TR><TD Width=150 Style=\"font-family: Arial; color:Black; font-size:9pt;  padding-left:2px; vertical-align:top; width:6.5in;height:0.35in;\" align='right'>Label</TD>" + newline;
                barTemplate += " <TD  class=\"font-family: Arial; color:Black; font-size:9pt;  padding-left:2px; vertical-align:top; width:6.5in;height:0.35in;\" Width=" + (ChartWidth + 25) + ">" + newline;
